...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    try {
        ...
        const conversations: Conversation[] = conversationsData.map((conv) => {
            ...
            const membersWithDetails: ConversationMemberWithDetails[] = conversationMembers.map((member) => {
            ...
            const otherMembers = membersWithDetails.filter((member) => member.workspace_member.user.id !== userId);
