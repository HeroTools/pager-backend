...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    try {
        ...
        const { data: stats, error } = await supabase.rpc('get_reaction_stats', { message_id_param: messageId });
        ...
        if (error) {
            ...
            const totalReactions = Object.values(reactionStats).reduce((sum: number, stat: any) => sum + stat.count, 0);

            return successResponse({
                reactions: Object.values(reactionStats),
                totalCount: totalReactions,
                userReacted: Object.values(reactionStats).some((stat: any) => stat.memberIds.includes(member.id)),
            });
        }
