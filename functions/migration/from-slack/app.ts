...
class ResilientSlackMigrator {
  ...
  constructor(client: any, workspaceId: string, currentUserId: string) {
    this.client = client;
  ...
  async migrateFromStream(zipStream: Readable, fileSize: number) {
    try {
      console.log(
        `Starting resilient migration for workspace: ${this.workspaceId}, size: ${fileSize} bytes`,
      );
  ...
  private async processZipEntriesResilient(zip: AdmZip) {
    ...
    const users: SlackUser[] = JSON.parse(usersEntry.getData().toString()).filter(
      (u: SlackUser) => !u.deleted && !u.is_bot,
    );
    ...
    // DON'T use a single transaction - migrate each section independently
    // This way if users fail, channels can still succeed

    // Migrate users (without transaction)
    await this.migrateUsersResilient(users);

    // Migrate channels (without transaction)
    ...
    // Migrate messages (with their own transactions per channel)
  ...
  private async migrateMessagesResilient(zip: AdmZip, channels: SlackChannel[]) {
    ...
    for (const channel of channels) {
      ...
      try {
        // Each channel's messages get their own transaction
        await this.client.query('BEGIN');

        const channelEntries = zip
          .getEntries()
          .filter(
            (e) => e.entryName.startsWith(`${channel.name}/`) && e.entryName.endsWith('.json'),
          );
        ...
        for (let i = 0; i < sortedMessages.length; i += batchSize) {
          ...
          for (const message of batch) {
            try {
              ...
              // Log individual message failures but continue
              console.warn(`Failed to migrate message in ${channel.name}:`, msgError.message);
            }
          }
        }
        ...
        for (const message of sortedMessages) {
          if (message.reactions) {
            try {
              ...
              // Log reaction failures but continue
  ...
  private async migrateUserWithAuth(slackUser: SlackUser) {
    ...
    const { rows: workspaceMemberRows } = await this.client.query(
    ...
    console.log(`Ensured workspace membership for user ${email} in workspace ${this.workspaceId}`);
  ...
  private async migrateDirectMessagesResilient(zip: AdmZip, directMessages: DirectMessage[]) {
    ...
    for (const dm of directMessages) {
      ...
      try {
        await this.client.query('BEGIN');
        await this.migrateDirectMessageMessages(zip, dm, conversationId);
        await this.client.query('COMMIT');
      } catch (error) {
        await this.client.query('ROLLBACK');
  ...
  private async migrateDirectMessageMessages(
    zip: AdmZip,
    dm: DirectMessage,
      ...
      for (const slackUserId of reaction.users) {
        ...
        try {
          await this.client.query(
            ...
            [this.workspaceId, messageId, workspaceMembers[0].id, reaction.name],
          );
...
export const handler = withCors(
  async (event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> => {
    ...
    try {
      ...
      // Download file as stream from Supabase Storage
      const { stream, size } = await downloadFileFromStorageAsStream(storageKey);

      console.log(`Downloaded file stream from storage: ${size} bytes`);

      // Run resilient migration
      const migrator = new ResilientSlackMigrator(client, workspaceId, userId);
      const results = await migrator.migrateFromStream(stream, size);

      // Clean up uploaded file from storage
      try {
        await supabase.storage.from('files').remove([storageKey]);
      ...
      return successResponse({
        success: true,
        results,
        message: `Migration completed! ${results.usersCreated} users, ${results.channelsCreated} channels, ${results.conversationsCreated} conversations, ${results.messagesImported} messages imported.`,
      });
    } catch (error) {
